...
void drawMultipleCards(int numCards, int player, struct gameState *state)
{
...
int baronEffect(int choice1, struct gameState *state, int currentPlayer)
{
  ...
  if (choice1 > 0) { //Boolean true or going to discard an estate
      ...
      while(card_not_discarded) {
          if (state->hand[currentPlayer+1][p] == estate) { //Found an estate card!
              state->coins += 4;//Add 4 coins to the amount of coins
              state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][p];
              //discardCount was normally increased here
...
if (choice1)
  ...
  else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
  {
      ...
      //draw 4
      drawMultipleCards(4, currentPlayer, state);
...
int ambassadorEffect(int i, int j, int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
  ...
  state->supplyCount[state->hand[currentPlayer][choice1]] += choice1;
...
int tributeEffect(int i, struct gameState *state, int currentPlayer, int nextPlayer, int tributeRevealedCards[])
{
  ...
  for (i = 0; i <= 2; i ++) {
      if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { //Treasure cards
          ...
          drawMultipleCards(2, currentPlayer, state);
      }
      else { //Action Card
...
int mineEffect(int i, int j, int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mineEffect(i, j, choice1, choice2, state, handPos, currentPlayer);
    ...
    case tribute:
        return tributeEffect(i, state, currentPlayer, nextPlayer, tributeRevealedCards);

    case ambassador:
        return ambassadorEffect(i, j, choice1, choice2, state, handPos, currentPlayer);
